...
namespace OutlookAddIn
{
    public partial class Ribbon2
    {
        ...
        private void htmlBody_Click(object sender, RibbonControlEventArgs e)
        {
            ...
            if (mailItem != null)
            {
                ...
                copy.To = Globals.ThisAddIn.Application.Session.CurrentUser.AddressEntry.Address;
                copy.Subject = "PROG SEND";

                // Save a copy before we send
                //Word.Document doc = inspector.WordEditor as Word.Document;
                //object path =@"";
                //object format = Word.WdSaveFormat.wdFormatFilteredHTML;

                //doc.SaveAs2(ref path, ref format);
                //path = @"";
        ...
        private void WordBody_Click(object sender, RibbonControlEventArgs e)
        {
            Outlook.Application _application = Globals.ThisAddIn.Application;
            string rootFolderPath = _application.Session.DefaultStore.GetRootFolder().FolderPath;
            Outlook.NameSpace outlookNameSpace = _application.GetNamespace("MAPI");
            ...
            foreach (Outlook.MailItem mailItem in sentfolder.Items)
            {
                if (mailItem != null)
                {
                    ...
                    doc.SaveAs(@"D:\Edward\Project\MSDNProject\MSDNProject\OutlookAddIn\bin\Debug\"+"test"+i.ToString()+".docx");
                    i++;
                }
            }            
        }
